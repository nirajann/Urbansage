...
const AdminOrderPage = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:4000/order/getorderss`,config)
            .then((res) => {
                setOrders(res.data);
            })
    ...
    const handleVerifyOrder = (orderId) => {
        setOrders(Orders.filter((order) => order._id !== orderId));
        axios.put(`http://localhost:4000/order/${orderId}`, { status: true }, {
            headers: {
                Authorization: `bearer ${window.localStorage.getItem('token')}`
            }
        })
                                ...
                                Orders.map((order) => {
                                    return (
                                        ...
                                        <tr ><td>{order.Hostelid.name}</td>
                                            <td>{order._id}</td>
                                            <td>RS {order.Hostelid.cheapestPrice}</td>
                                            <td>
                                
                                                <button className="btn btn-success mx-2" onClick={()=> handleVerifyOrder(order._id)}>Verify</button>
